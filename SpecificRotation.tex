...
\begin{figure}[h] \label{img:1}  % Begin a figure environment to insert image, the 'h' tells the LaTeX compiler to place the figure 'here' (if not used, it will place it where it deems suitable)
...
% This requires the minted package along with the Pygments python library. Check dkpinto-report.cls for more details.
% Inserting code directly 
%\begin{minted}{python}
%for i in range(5):
%    print("Hello World")
%\end{minted}
% Inserting code from a source file
%inputminted{name_of_source_code.py}{python}
